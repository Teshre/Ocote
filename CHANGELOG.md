...
- **Carpetas con acentos en producción — causa raíz encontrada (lsof + locale C)**: tras el primer fix de normalización, el error mutó a "Operación fuera del directorio permitido: '…Caf\xc3\xa9…'". El path contenía literalmente el TEXTO `\xc3\xa9` (no el carácter `é`): `lsof` —que usamos para leer el cwd del proceso del shell— **escapa los bytes no-ASCII como `\xNN` cuando corre bajo locale C**. El `.app` lanzado desde Finder no hereda `LANG` (locale C) → lsof corrompe el path → explorador falla. En dev (lanzado desde terminal con locale UTF-8) nunca se reproducía. Fix doble en `get_shell_cwd`: (1) spawn de lsof con `LC_ALL/LANG=en_US.UTF-8`, (2) `decode_lsof_escapes()` decodifica `\xNN` literales como defensa (con tests unitarios NFC/NFD). Verificado reproduciendo: `LC_ALL=C lsof` → `Caf\xc3\xa9`, `LC_ALL=en_US.UTF-8 lsof` → `Café`.
