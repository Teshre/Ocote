...
- **lsof y locale C (CRÍTICO — la causa raíz del bug de acentos en prod)**: `lsof` escapa los bytes no-ASCII como TEXTO literal `\xNN` cuando corre bajo locale C. Un `.app` lanzado desde Finder NO hereda `LANG` (locale C) → lsof devolvía `Caf\xc3\xa9` (texto) en vez de `Café` → paths corruptos en todo el flujo del explorador. En dev nunca se reproduce (la terminal exporta locale UTF-8). Fix en `get_shell_cwd` (pty.rs): spawn de lsof con `LC_ALL/LANG=en_US.UTF-8` + `decode_lsof_escapes()` como fallback (tests en `lsof_escape_tests`). **REGLA: todo `Command::new` cuya salida contenga paths debe fijar `LC_ALL` explícito** — el entorno de un .app de Finder es mínimo. Si un error muestra `\xNN`, distinguir si son bytes mostrados o texto literal en el string: cambia el diagnóstico por completo.
